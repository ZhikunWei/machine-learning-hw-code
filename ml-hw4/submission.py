...
class ActorCritic(nn.Module):
    ...
    def train_net(self):
        ...
        vt = self.v(s)
        vt_prime = self.v(s_prime)
        A_sa = (r + self.gamma * vt_prime - vt).squeeze(dim=1)
        v_loss = 0.5 * A_sa ** 2

        prob = self.pi(s)
        policies = torch.stack([p[action] for p, action in zip(prob, a)])
        p_loss = - policies.log() * A_sa
    ...
    for n_epi in range(max_episodes):
        ...
        if n_epi % print_interval == 0 and n_epi != 0:
            ...
            with open('log.txt', 'a') as f:
                f.write(str(n_epi) + ' ' + str(score / print_interval) + ' \n')
            score = 0.0
    env.close()
